...
public class ArticleLuceneService implements IArticleLucene {
    ...
    private Document toDoc(Article article) {
        ...
        doc.add(new Field("id", article.getId().toString(), Store.YES, Index.NO));
        doc.add(new Field("content", Jsoup.parse(article.getContent()).text(), Store.NO, Index.ANALYZED_NO_NORMS));
        return doc;
    }
